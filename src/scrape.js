...
async function fetchProfileImage(profileUrl) {
  ...
  const headshot = $("div.largeHeadshot.rounded-circle.m-3.bg-light").first();

  if (!headshot.length) {
    return null;
  }

  const style = headshot.attr("style") || "";
...
export async function scrapeTop10() {
  ...
  for (const row of rowCandidates) {
    ...
    rows.push({
      ...
      team: getCellText(cells, map.get("TEAM")),
  ...
  for (const player of top10) {
    ...
    try {
      player.imageUrl = await fetchProfileImage(player.profileUrl);
    } catch (error) {
