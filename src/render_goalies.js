...
function buildSvg(payload) {
  ...
  svg += `<text x=\"${titleCenterX}\" y=\"${topMidY + 12}\" fill=\"${COLORS.fg}\" text-anchor=\"middle\" font-size=\"40\" font-family=\"DejaVu Sans, Arial, sans-serif\" font-weight=\"700\">NZIHL GOALIE LEADERS</text>`;
  ...
  for (let index = 0; index < payload.goalies.length; index += 1) {
    ...
    svg += `<text x=\"${teamX}\" y=\"${cy + 9}\" fill=\"${COLORS.sub}\" font-size=\"31\" font-family=\"DejaVu Sans, Arial, sans-serif\" font-weight=\"700\">${escapeXml(clampText(String(goalie.team || ""), 22))}</text>`;
